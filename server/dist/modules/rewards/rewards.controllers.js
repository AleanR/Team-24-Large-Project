...
exports.redeemReward = exports.getRewards = void 0;
const users_model_1 = require("../users/users.model");
const rewards_model_1 = require("./rewards.model");
...
// ── Send voucher email ────────────────────────────────────────────────────────
async function sendVoucherEmail(to, rewardName, voucherCode) {
...
const getRewards = async (req, res) => {
    try {
        const rewards = await (0, rewards_model_1.getActiveRewards)();
...
const redeemReward = async (req, res) => {
    try {
        ...
        const [user, reward] = await Promise.all([
            users_model_1.UserModel.findById(id),
            (0, rewards_model_1.getRewardById)(rewardId),
        ]);
        if (!user)
        ...
        sendVoucherEmail(user.email, reward.name, voucherCode).catch(err => console.error('Voucher email failed:', err.message));
...
//# sourceMappingURL=rewards.controllers.js.map